...
def write_summary(output_dir: Path, args: argparse.Namespace, x: np.ndarray, y: np.ndarray, train_idx: np.ndarray, val_idx: np.ndarray, metrics: dict, history: list[dict]) -> None:
    ...
    lines = [
        ...
        f"- Final validation accuracy: {final.get('val_accuracy', 0.0):.4f}",
        f"- Final macro F1: {final.get('macro_f1', 0.0):.4f}",
        "",
        "## Label Distribution",
        "",
    ]
    ...
    (output_dir / "metrics.json").write_text(json.dumps({"final": final, "per_class": metrics["per_class"]}, indent=2), encoding="utf-8")
